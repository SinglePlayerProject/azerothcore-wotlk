...
public:
    ...
    bool CreateRobotCharacter(uint32 pmAccountID, uint8 pmCharacterClass, uint8 pmCharacterLevel);
    bool LoginRobot(uint32 pmAccountID, uint64 pmGUID);
    void LogoutRobots();
    bool LogoutRobot(Player* pmPlayer);
    void UninviteRobot(Player* pmPlayer);
    bool ProcessRobots();
...
public:
    ...
    uint8 managerState;
    std::map<uint8, std::vector<uint8>> availableRaces;
    std::unordered_map<uint32, std::string> robotNameMap;
    std::set<uint64> creatingCharacterGUIDSet;
    int32 checkDelay;

    std::unordered_map<uint8, std::unordered_map<uint8, std::string>> classCharacterTypeNameMap;
