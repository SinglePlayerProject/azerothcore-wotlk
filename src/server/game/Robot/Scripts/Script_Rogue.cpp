...
bool Script_Rogue::DPS_Common(Unit* pmTarget)
{
	...
	if (energy > 25)
	{
		...
		{
			if (sourceAI->CastSpell(pmTarget, "Kick", MELEE_MIN_DISTANCE))
			{
				return true;
			}
		}
	...
	if (energy > 50)
	{
		...
		{
			sourceAI->CastSpell(pmTarget, "Eviscerate", MELEE_MIN_DISTANCE);
			return true;
		}
		if (sourceAI->CastSpell(pmTarget, "Sinister Strike", MELEE_MIN_DISTANCE))
		{
			return true;
		}
	}
...
bool Script_Rogue::Attack_Common(Unit* pmTarget)
{
	...
	if (sourceAI->FindSpellID("Cheap Shot") > 0)
	{
		if (targetDistance > ROGUE_PREPARE_DISTANCE)
		{
			...
			{
				if (sourceAI->CastSpell(me, "Stealth", MELEE_MIN_DISTANCE, true))
				{
					return true;
				}
			}
