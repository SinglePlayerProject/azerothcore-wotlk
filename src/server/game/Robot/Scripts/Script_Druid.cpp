...
bool Script_Druid::DPS_Feral(Unit* pmTarget)
{
    ...
    case ShapeshiftForm::FORM_CAT:
    {
        ...
        if (targetDistance < DRUID_PREPARE_DISTANCE)
        {
            if (energy > 20)
            {
                if (sourceAI->CastSpell(me, "Tiger's Fury", MELEE_MIN_DISTANCE, true))
                {
                    return true;
                }
            }
        ...
        if (energy > 35)
        {
            ...
            if (spellID != 0)
            {
                ...
                if (sourceAI->HasAura(pmTarget, "Rip", true))
                {
                    ...
                    {
                        sourceAI->CastSpell(pmTarget, "Ferocious Bite", MELEE_MIN_DISTANCE);
                        return true;
                    }
                }
                else
                {
                    if (urand(1, 5) <= comboPoints)
                    {
                        sourceAI->CastSpell(pmTarget, "Rip", MELEE_MIN_DISTANCE);
                        return true;
                    }
                }
            }
        ...
        if (energy > 45)
        {
            if (sourceAI->CastSpell(pmTarget, "Rake", MELEE_MIN_DISTANCE, true, true))
            {
                return true;
            }
            if (sourceAI->CastSpell(pmTarget, "Claw", MELEE_MIN_DISTANCE))
            {
                return true;
            }
        }
...
bool Script_Druid::Tank(Unit* pmTarget)
{
    ...
    case ShapeshiftForm::FORM_DIREBEAR:
    {
        ...
        if (rage > 100)
        {
            if (sourceAI->CastSpell(pmTarget, "Demoralizing Roar", MELEE_MIN_DISTANCE, true))
            {
                return true;
            }
            if (pmTarget->IsNonMeleeSpellCast(false))
            {
                if (sourceAI->CastSpell(pmTarget, "Bash", MELEE_MIN_DISTANCE))
                {
                    return true;
                }
            }
        ...
        if (validAttackerCount > 1)
        {
            ...
            if (rage > 200)
            {
                if (sourceAI->CastSpell(pmTarget, "Swipe", MELEE_MIN_DISTANCE))
                {
                    return true;
                }
            }
        ...
        if (rage > 200)
        {
            if (sourceAI->CastSpell(pmTarget, "Maul", MELEE_MIN_DISTANCE))
            {
                return true;
            }
        }
...
bool Script_Druid::Attack_Feral_Cat(Unit* pmTarget)
{
    ...
    if (sourceAI->FindSpellID("Pounce") > 0)
    {
        if (targetDistance > DRUID_PREPARE_DISTANCE)
        {
            sourceAI->BaseMove(pmTarget, MELEE_MIN_DISTANCE, true, false);
            if (targetDistance < DRUID_RANGE_DISTANCE)
            {
                if (sourceAI->CastSpell(me, "Prowl", MELEE_MIN_DISTANCE, true))
                {
                    return true;
                }
            }
...
bool Script_Druid::Healer(Unit* pmTarget)
{
    ...
    for (uint32 type = SPELL_AURA_NONE; type < TOTAL_AURAS; ++type)
    {
        ...
        for (std::list<AuraEffect*>::const_iterator auraIT = auraList.begin(); auraIT != auraList.end(); auraIT++)
        {
