...
bool Strategy_Solo_Normal::Battle()
{
    ...
    if (!result)
    {
        ...
        {
            if ((*it)->GetCreatureType() == CreatureType::CREATURE_TYPE_CRITTER)
            {
                continue;
            }
            if ((*it)->IsPet())
...
bool Strategy_Solo_Normal::Stroll()
{
    ...
    Player* me = sourceAI->sourcePlayer;
    std::list<Unit*> strollTargets;
    Trinity::AnyUnitInObjectRangeCheck u_check(me, DEFAULT_VISIBILITY_DISTANCE);
    Trinity::UnitListSearcher<Trinity::AnyUnitInObjectRangeCheck> searcher(me, strollTargets, u_check);
    me->VisitNearbyObject(DEFAULT_VISIBILITY_DISTANCE, searcher);
    int randomIndex = 0;
    randomIndex = urand(0, strollTargets.size() - 1);
    int checkIndex = 0;
    for (std::list<Unit*>::iterator it = strollTargets.begin(); it != strollTargets.end(); it++)
    {
        if (checkIndex == randomIndex)
        {
            me->GetMotionMaster()->Clear();
            me->StopMoving();
            me->GetMotionMaster()->MoveFollow((*it), 1, 0);
            result = true;
            break;
        }
        checkIndex++;
