...
void MarketerConfig::LoadMarketerConfig()
{
    LoadInitial(MARKETER_CONFIG_FILE_NAME);
    Reload();
    enable = GetIntDefault("Enable", 0);
    reset = GetIntDefault("Reset", 0);
    hordeFaction = GetIntDefault("HordeAuctionFaction", 104);
    neutralFaction = GetIntDefault("NeutralAuctionFaction", 120);
    allianceFaction = GetIntDefault("AllianceAuctionFaction", 55);
    hordeAuctioneerGUID = GetIntDefault("HordeAuctioneerGUID", 24708);
    neutralAuctioneerGUID = GetIntDefault("NeutralAuctioneerGUID", 570);
    allianceAuctioneerGUID = GetIntDefault("AllianceAuctioneerGUID", 126);
    if (enable == 0)
    {
        return;
    }
